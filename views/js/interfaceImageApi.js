$(document).ready(function() {
    ...
    var countInfo = $("#info-count-image");
    var sizeInfo = $("#info-size-image");

    // ul-список, содержащий миниатюрки выбранных файлов
    var imgList = $('#image-list');

    // Контейнер, куда можно помещать файлы методом drag and drop
    var dropBox = $('#image-container');
    ...
    fileInput.damnUploader({
        ...
        //limit: 5,
        limit: +$('#upload-box').attr('data-max'),
        ...
        onAllComplete: function() {
            ...
            var goodId = $('#id_good').val();

            $.ajax({
                type: 'POST',
                url: '/ajax/get_user_layouts.php',
                data: { },
                dataType: 'JSON',
                success: function(data){
                    if (data.images != 0) {
                        $('#show-layouts').empty();
                        $('#show-layouts').html(data.images);
                        if (data.avalib > 0) {
                            $('#no-layouts > div > span').text(data.avalib);
                            $('#no-layouts').show();
                            $('#upload-box').attr('data-max', data.avalib);
                        } else if (data.avalib == 0) {
                            $('#no-layouts').hide();
                        }
                    }
                }
    ...
    function addFileToQueue(file) {
        ...
        if(!file.fake) {
            ...
            var imageType = /image.*/;
            if (!file.type.match(imageType) && !file.name.match(/.*.pdf|.*.psd|.*.cdr|.*.eps/)) {
            ...
            if(file.name.match(/.*.psd/)){
                var file_type='psd';
            }
            if(file.name.match(/.*.cdr/)){
                var file_type='cdr';
            }
            if(file.name.match(/.*.eps/)){
                var file_type='eps';
            }
            if(file.name.match(/.*.tif/)){
                var file_type='tiff';
            }
            if(file.name.match(/.*.tiff/)){
                var file_type='tiff';
            }
            ...
            if($.support.fileReading) {
                ...
                reader.onload = (function(aImg) {
                    return function(e) {
                        ...
                        if(file_type=='pdf' || file_type=='psd' || file_type=='cdr' || file_type=='eps' || file_type=='tiff'){
                            aImg.attr('src', '/pic/'+file_type+'.png');
                        }else{
    ...
    $("#upload-all-image").click(function(event) {
    ...
    $("#cancel-all-image").click(function(event) {
        event.preventDefault();
